...
CREATE OR REPLACE VIEW view__affected_by_events AS
    SELECT
        ...
        CASE 
            WHEN events.affects_department2 THEN classes_department.id=events_department.id
            WHEN events_department.department1_id IS NULL THEN classes_department.id=events_department.id
            WHEN classes_department.id=events_department.id
                THEN
                    COALESCE(classes_department.department1_id, classes_department.id)=COALESCE(events_department.department1_id, events_department.id)
                    AND (
