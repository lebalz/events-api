...
export const createEventGroup = async (props: (Partial<Prisma.EventGroupUncheckedCreateInput> & {userIds: string[], eventIds: string[]})) => {
...
describe('all groups of user', () => {
	test('can get all groups', async () => {
		...
		await expect(EventGroups.allOfUser(alice)).resolves.toEqual(expect.arrayContaining(
			[groupMono, groupShared].map(g => prepareEventGroup(g))
		));
	});
});
...
describe('all events of group', () => {
	test('can get all events', async () => {
		...
		await expect(EventGroups.events(alice, group.id)).resolves.toEqual(
			[prepareEvent(event)]
		);
	});
});
...
describe('all groups where an event is part of', () => {
	test('can get all groups', async () => {
		...
		await expect(EventGroups.allOfEvent(event)).resolves.toEqual(expect.arrayContaining(
			[groupMono, groupShared, groupBob].map(g => prepareEventGroup(g))
		));
	});
});
...
describe('internal: find raw event group model', () => {
	test('can raw record if allowed', async () => {
		...
		await expect(EventGroups._findRawModel(admin, group.id)).rejects.toEqual(new HTTP404Error('Not found'));
...
describe('create group', () => {
	test('user can create a group', async () => {
		const alice = await createUser({});
		const event = await createEvent({ authorId: alice.id });
		await expect(EventGroups.createModel(
			alice,
			{
				name: 'Test',
...
describe('clone group', () => {
	test('user can clone group and it\'s events', async () => {
		...
		const pubEvent = await createEvent({ authorId: alice.id, description: 'Published', state: 'PUBLISHED'});
		const eventBob = await createEvent({ authorId: bob.id, description: 'Bob', state: 'PUBLISHED'});
		const eventBobChild = await createEvent({ authorId: bob.id, description: 'Bob Child', parentId: eventBob.id});

		const group = await createEventGroup({ userIds: [alice.id, bob.id], eventIds: [event.id, pubEvent.id, eventBob.id, eventBobChild.id] });
