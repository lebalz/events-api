...
describe('linkToUntis', () => {
    test('user can link self to untisId', async () => {
        ...
        const untis = await prisma.untisTeacher.create({ data: { id: 42, active: true, longName: '', name: 'infinity', title: '' } });
        await expect(Users.linkToUntis(user, user.id, 42)).resolves.toEqual({...user, untisId: 42, updatedAt: expect.any(Date)});
    });
...
describe('setRole', () => {
    test('admin can give admin roles', async () => {
        ...
        await expect(Users.setRole(admin, user.id, Role.ADMIN)).resolves.toEqual({...user, role: Role.ADMIN, updatedAt: expect.any(Date)});
    });
    test('admin can revoke admin roles', async () => {
        ...
        await expect(Users.setRole(admin, user.id, Role.USER)).resolves.toEqual({...user, role: Role.USER, updatedAt: expect.any(Date)});
    });
...
describe('affectedEvents', () => {
    ...
    test('admin can not get affected events of unknown user', async () => {
        ...
        await expect(Users.affectedEvents(maria, 'cc28810e-4b91-41f0-b6ab-20ac71cd2152')).rejects.toEqual(new HTTP404Error('User not found'));
    });
});