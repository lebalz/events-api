    ...
    const result = await prisma.$queryRaw<Event>(
        Prisma.sql`WITH drange AS (
            SELECT 
             ...
             FROM events WHERE id=${eventId} AND (events.state='PUBLISHED' OR events.author_id=${userId} OR 'ADMIN' = (SELECT role FROM users WHERE id=${userId}))
        ) 
        SELECT untis_lessons.*, untis_teachers.name as teacher
        FROM  untis_lessons
            ...
            drange /* append calculated values to each row for easyer access*/
        where
                ...
                AND untis_classes.name IN (SELECT unnest(drange.classes))
